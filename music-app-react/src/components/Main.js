            ...
            <main className={style.main}>
                <ul>
                    ...
                    //user is not logged in => no liked songs
                     <h2 className={style.nothingToSee}>Nothing to see here... <a href="/login">Login</a> or <a href="/register">register</a> to like some songs</h2>}
                    {/* if there are no liked songs but user is logged in */}
                    {!this.props.loading && this.props.songs?.length === 0 && this.props.inputGenre === 'Liked songs' &&
                        <h2 className={style.nothingToSee}>Nothing to see here... Like some songs!</h2>
                    }
                    {!this.props.loading && this.props.songs?.length === 0 && this.props.inputGenre !== 'Liked songs'&&
                        <h2 className={style.nothingToSee}>No results...</h2>
                    }
                    {this.props.loading &&
                        <h2 className={style.nothingToSee}>Loading...</h2>
                    }
                </ul>
                {this.state.isOpen  &&
                    <MusicPlayer
